...
static void add_sample(double val) {
  ...
  while (pos > 0 && values[pos - 1] > values[pos]) {
    const double temp = values[pos - 1];
...
static void clear(void) {
  int x = sink;
  const unsigned int incr = cache_block / sizeof(int);
  if (!cache_buf) {
  ...
  const int *cptr = (int *)cache_buf;
  const int *cend = cptr + (cache_bytes / sizeof(int));
  while (cptr < cend) {
    x += *cptr;
    cptr += incr;
  }
 ...
 */
double fcyc(const test_funct f, void *argp) {
  double result;
  init_sampler();
  if (compensate) {
    do {
      ...
      start_comp_counter();
      f(argp);
      const double cyc = get_comp_counter();
      add_sample(cyc);
 ...
 *     Default = 0
 */
void set_fcyc_clear_cache(const int clear) { clear_cache = clear; }

/*
 * set_fcyc_cache_size - Set size of cache to use when clearing cache
 *     Default = 1<<19 (512KB)
 */
void set_fcyc_cache_size(const int bytes) {
 ...
 *     Default = 32
 */
void set_fcyc_cache_block(const int bytes) { cache_block = bytes; }
 ...
 *     Default = 0
 */
void set_fcyc_compensate(const int compensate_arg) {
 ...
 *     Default = 3
 */
void set_fcyc_k(const int k) { kbest = k; }
 ...
 *     Default = 20
 */
void set_fcyc_maxsamples(const int maxsamples_arg) {
 ...
 *     Default = 0.01
 */
void set_fcyc_epsilon(const double epsilon_arg) { epsilon = epsilon_arg; }
