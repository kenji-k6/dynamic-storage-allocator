...
#define CHUNKSIZE                                                              \
  (1 << 11) // extend heap by this amount in bytes, also size of initial free
            // block
...
#define GET_SIZE(p)                                                            \
  (GET(p) & ~0x7) // mask out last 3 bits, since those are needed for meta data
#define GET_ALLOC(p)                                                           \
  (GET(p) & 0x1) // mask out all but last bit, since that is the allocated bit

/*Given block-pointer bp compute adress of its header and footer*/
#define HDRP(bp) ((char *)(bp) - WSIZE) // header is 4 bytes before bp
#define FTRP(bp)                                                               \
  ((char *)(bp) + GET_SIZE(HDRP((char *)(bp))) -                               \
   (2 * WSIZE)) // footer follows directly after  payload, since size includes
                // hdr/ftr we subtract DSIZE

/* Given block-pointer bp, compute adress of next and previous blocks, bp should
 * be block pointer, not header/footer*/
#define NEXT_BLKP(bp)                                                          \
  ((char *)(bp) + GET_SIZE(HDRP((bp)))) // next block starts at end of ftr
#define PREV_BLKP(bp) ((char *)(bp) - GET_SIZE(((char *)(bp) - (2 * WSIZE))))

/* Macros for Explicit/Segregated free-list */

#define NEXT_FREE(bp)                                                          \
  (*(char **)(bp)) // returns pointer to next free block in free list
#define PREV_FREE(bp)                                                          \
  (*(char **)((char *)bp +                                                     \
              WSIZE)) // returns pointer to prev free block in free list
#define NUMSEGS 12
...
int mm_init(void) {
  ...
  segs = heap_listp;
  PUT(heap_listp,
      PACK((14 * WSIZE), 1)); // allocate space for the segregated lists
  ...
  heap_listp +=
      (1 * WSIZE); // heap_listp now points to payload of prologue block
...
void *mm_malloc(size_t size) {
  ...
  const size_t asize =
      ALIGN(size) +
      (2 * WSIZE); // adjust block size to include overhead and alignment reqs
