 ...
 */
double ftimer_itimer(const ftimer_test_funct f, void *argp, const int n) {

  init_etime();
  const double start = get_etime();
  for (int i = 0; i < n; i++)
    f(argp);
  const double tmeas = get_etime() - start;
  return tmeas / n;
}
 ...
 */
double ftimer_gettod(const ftimer_test_funct f, void *argp, const int n) {
  ...
  double diff =
      1E3 * (double)(etv.tv_sec - stv.tv_sec) + 1E-3 * (etv.tv_usec - stv.tv_usec);
  diff /= n;
  return (1E-3 * diff);
}
...
static double get_etime(void) {
  ...
  return (double)((double)(first_p.it_value.tv_sec - r_curr.it_value.tv_sec) +
