...
char err_msg[MAXLINE]; /* for whenever we need to compose an error message */

/* Directory where default tracefiles are found */
static char glbl_tracedir[MAXLINE] = TRACEDIR;
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "f:t:hvVglc:")) != EOF) {
    ...
    case 't':                  /* Directory where the traces are located */
      if (num_tracefiles == 1) /* ignore if -f already encountered */
        break;
      strcpy(glbl_tracedir, optarg);
      if (glbl_tracedir[strlen(glbl_tracedir) - 1] != '/')
        strcat(glbl_tracedir, "/"); /* path always ends with "/" */
      break;
  ...
  if (run_libc) {
    ...
    for (i = 0; i < num_tracefiles; i++) {
      trace = read_trace(glbl_tracedir, tracefiles[i]);
  ...
  for (i = 0; i < num_tracefiles; i++) {
    ...
    if (mm_stats[i].valid) {
      if (verbose > 1)
        printf("efficiency, ");
      mm_stats[i].util = eval_mm_util(trace);
      speed_params.trace = trace;
...
static int add_range(range_t **ranges, char *lo, int size, int tracenum,
  ...
  if (!IS_ALIGNED(lo)) {
    snprintf(msg, MAXLINE, "Payload address (%s) not aligned to %d bytes", lo,
             ALIGNMENT);
  ...
  if ((lo < (char *)mem_heap_lo()) || (lo > (char *)mem_heap_hi()) ||
    ...
    snprintf(msg, MAXLINE, "Payload (%s:%s) lies outside heap (%p:%p)", lo, hi,
  ...
  for (p = *ranges; p != NULL; p = p->next) {
    if ((lo >= p->lo && lo <= p->hi) || (hi >= p->lo && hi <= p->hi)) {
      snprintf(msg, MAXLINE,
               "Payload (%s:%s) overlaps another payload (%s:%s)\n", lo, hi,
               p->lo, p->hi);
      malloc_error(tracenum, opnum, msg);
      return 0;
    }
...
static trace_t *read_trace(char *tracedir, char *filename) {
  ...
  if ((tracefile = fopen(path, "r")) == NULL) {
    if (snprintf(err_msg, MAXLINE, "Could not open %s in read_trace", path) >=
        MAXLINE) {
  ...
  fclose(tracefile);
  assert(max_index == (unsigned)trace->num_ids - 1);
  assert((unsigned)trace->num_ops == op_index);
